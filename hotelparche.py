...
class Cliente:
    def verificar_estado_habitacion(self):
        numero_habitacion = int(input("Ingresa el número de la habitación que deseas verificar: "))
        GestorHabitaciones().mostrar_estado_habitacion(numero_habitacion)
...
class GestorReservas:
    ...
    def elegir_habitacion(self, habitaciones):
        while True:
            try:
                self.mostrar_habitaciones_disponibles(habitaciones)
    ...
    def realizar_reserva(self, usuario, fecha_inicio, fecha_fin, habitaciones):
        if fecha_inicio > fecha_fin:
...
class GestorHabitaciones:
    def mostrar_estado_habitacion(self, numero_habitacion):
...
fecha_inicio, fecha_fin = gestor_reservas.solicitar_fechas()
...
habitacion_elegida = gestor_reservas.elegir_habitacion(habitaciones)
...
if habitacion_elegida:
    ...
    if usuario_actual:
        gestor_reservas.realizar_reserva(usuario_actual, fecha_inicio, fecha_fin, [habitacion_elegida])
    else:
...
while True:
    ...
    if opcion_cliente == 1:
        cliente.verificar_estado_habitacion()
    elif opcion_cliente == 2:
        if usuario_actual:
            cliente.cancelar_reserva(usuario_actual)
        else:
